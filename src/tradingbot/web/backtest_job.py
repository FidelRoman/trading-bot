...
class BacktestJob:
    ...
    def run_sync(self, source: str, months: int, equity: float, spread_pips: float) -> None:
        """Corre en un thread (asyncio.to_thread). start_allowed() debe ser True."""
        started = datetime.now(timezone.utc).isoformat(timespec="seconds")
        try:
            df, source_label = self._load_data(source, months)
    ...
    def _load_data(self, source: str, months: int):
        if source == "synthetic":
            return synthetic_df(days=months * 30), f"Sintético {months} meses"
        if source == "csv":
            if not UPLOAD_CSV.exists():
                raise RuntimeError("No hay CSV subido: usa 'Subir CSV' primero")
            return load_csv(UPLOAD_CSV), "CSV subido"
        if source == "fxcm":
            ...
            df = download_history(self.broker, months, HISTORY_DIR, progress=self._set_note)
            return df, f"FXCM {months} meses"
